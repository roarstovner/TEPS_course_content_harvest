...
4. **`fetch_html_with_checkpoint()`**: Downloads HTML (skips already-downloaded courses)
5. **`extract_fulltext()`**: Extracts clean text from HTML using CSS selectors
...
To add support for a new institution, you need to modify **two key files**:
...
For one particularly gnarly selector, asking an LLM with thinking enabled to return "the full info about Emneplan, but without extraneous elements" got the right selector.
...
**URL pattern tokens** you can use:
- `{year}` - Year (e.g., "2024")
...
Expand a little bit at a time, for example by year: `filter(Årstall %in% c(2017, 2018))` Add more years as the pipeline works.
...
output <- tribble(
  ...
  "oslomet_ABC123_2024_autumn_1", "https://student.oslomet.no/...", "<html>...</html>", TRUE, "ABC123 Course Description...",
  "oslomet_XYZ456_2024_spring_1", "https://student.oslomet.no/...", "<html>...</html>", TRUE, "XYZ456 Another course..."
)
output
```
...
Extracts clean text from raw HTML using institution-specific CSS selectors.
...
- Check `config/selectors.yaml` matches the HTML structure
