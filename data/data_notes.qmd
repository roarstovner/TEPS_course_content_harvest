...
summary_tbl <- df |>
  ...
  summarise(
    ...
    `Fulltext OK`= sum(!is.na(fulltext) & nchar(fulltext) > 0),
    has_plan_id  = sum(!is.na(plan_content_id)),
    `Median chars` = round(median(nchar(fulltext[!is.na(fulltext) & nchar(fulltext) > 0]), na.rm = TRUE)),
...
**Total**: `r total_rows` rows, `r total_fulltext` with fulltext (`r total_rate`), `r total_plans` unique plans.
...
- **Litteraturlister (reading lists)** are included in the fulltext — only the timestamp line is stripped, not the reading list content itself (book titles, authors, ISBNs, etc.). This differs from NLA, which truncates at "Digital litteraturliste" to remove reading lists entirely
...
- **Missing plans**: A few 2022 courses (e.g., 2EXPHIL51, 2PEDL51-2-1, 2ENL512-3) return pages with no course plan content. These are not caught by the "Emnesøket" filter since the page structure differs; fulltext may contain navigation/header text instead of plan content (see #123)
...
- **0% fulltext rate**: No reliable matching between DBH course codes and website course codes (see #85, #89, #90)
...
- **31.4% overall success rate** (917/2920 with fulltext); 36% of unique URLs succeed (1053/2920 HTML fetched)
