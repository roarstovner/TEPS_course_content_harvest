...
w("- Course plans are published as PDFs (labeled \"Oahppoplána\") linked from course pages at `samas.no/se/oahput`")
w("- Text extracted with `pdftools::pdf_text()`, not HTML scraping")
w("- 128/130 rows with fulltext (98.5%). 2 courses (PED-6501, SAAL 2) had no PDF on their pages")
w("- Very high dedup (95.3%) with only 6 unique plans — multiple course codes share the same PDF document (program-level plans covering several courses)")
w("- Highest median content (26,959 chars) because PDFs contain full program plans")
