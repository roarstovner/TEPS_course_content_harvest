...
local({
  ...
  get_courses_std <- function(path="data/cache/courses_std.RDS"){
    ...
    miss <- setdiff(need, names(x)); if (length(miss)) stop("Cache mangler: ", paste(miss, collapse=", "))
    x
  }
  df0 <- get_courses_std()
  ...
  tpl_sub <- function(pattern, env){
    out <- pattern
    for (k in names(env)) out <- gsub(paste0("\\{",k,"\\}"), ifelse(is.na(env[[k]]),"",env[[k]]), out, perl=TRUE)
    out
  }
  safe_read_yaml <- function(path){
    tr <- try({ con<-file(path,"r",encoding="UTF-8"); on.exit(close(con),add=TRUE); yaml::read_yaml(con) }, TRUE)
    if (!inherits(tr,"try-error") && !is.null(tr)) return(tr)
    raw <- readBin(path,"raw",file.info(path)$size); txt <- rawToChar(raw); Encoding(txt) <- "unknown"
    txt <- sub("^\ufeff","",txt); txt <- gsub("\r\n?","\n",txt); txt <- gsub("\t","  ",txt, fixed=TRUE)
    yaml::yaml.load(txt)
  }
  ...
  if (is.null(inst) || !nzchar(as.character(inst$url_pattern)[1])) stop("Mangler url_pattern i YAML for ", inst_short)
  pattern <- as.character(inst$url_pattern)[1]
  
  # --- tokens (vi fyller alle; pattern plukker de som trengs) ---
