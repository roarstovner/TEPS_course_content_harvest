# ============================================================
# R/oslomet/parse_html_oslomet.R
# Parser for OsloMet (EPN/Liferay-based course pages)
# Mirrors the structure of the Hiof parser
# ============================================================
...
parse_html_oslomet <- function(file_path, cfg = NULL) {
  # 1. Read HTML safely
  ...
  main <- html_element(page, "main, .epn-side, #content, article")
  if (length(main) == 0) main <- page
  
  # 3. Remove navigation and boilerplate
  exclude <- c("nav", "footer", "header", ".breadcrumbs", ".menu", ".social", ".print-link")
  ...
  # 5. Extract key facts (if present in a table or info box)
  facts <- html_elements(main, ".course-facts, .epn-facts, table, dl")
  if (length(facts)) {
  ...
  # 8. Remove footer/legal text
  remove_patterns <- c("(?i)personvern", "(?i)cookies", "(?i)tilgjengelighet", "(?i)osloMet")
