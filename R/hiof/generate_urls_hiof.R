...
local({
  ...
  tpl_sub <- function(pat, env){ out <- pat; for (k in names(env)) out <- gsub(paste0("\\{",k,"\\}"), ifelse(is.na(env[[k]]),"",env[[k]]), out, perl=TRUE); out }
  cfg <- get_cfg("config/institutions.yaml"); inst <- cfg$institutions[[inst_short]]
  ...
  sem_url <- function(style, hv){
    ...
    if (style %in% c("oslomet","plain_url")) { lbl <- ifelse(hv=="H","H??ST", ifelse(hv=="V","V??R","")); return(URLencode(lbl, reserved=TRUE)) }
  ...
  expand_one <- function(i){
    ...
    wants <- switch(
      ...
      V    = "V",
      hv   = have,
      "next" = if (have == "V") "V" else "H",  # <??? n??kkelen er sitert
      have  # default
    )
  ...
  if (any(grepl("\\{[^}]+\\}", out$url))) warning("Uerstattede tokens ??? sjekk YAML. Eksempel: ", grep("\\{[^}]+\\}", out$url, value=TRUE)[1])
  ...
  for (k in names(split_out)){
    o <- split_out[[k]]
    write.csv(o[,c("course_code_norm","year","hv","url")], file.path(outd, sprintf("course_urls_%s_%s_%s.csv", inst_short, k, ts)), row.names=FALSE, fileEncoding="UTF-8")
    writeLines(o$url[nchar(o$url)>0], file.path(outd, sprintf("course_urls_%s_%s_%s.txt", inst_short, k, ts)), useBytes=TRUE)
  }
