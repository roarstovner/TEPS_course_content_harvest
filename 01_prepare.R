...
# - Lage semester_hv (H/V/NA) – institusjonsnøytralt
# - Alias til institution_short fra config/institutions.yaml (robust med ASCII-nøkler)
...
#
# Viktig: Modul 01 gjør *ikke* URL-bygging, scraping eller generering av framtidsår.
# Den lager kun en renset og stabil basisfil (cache) for videre steg.

# ========== 0. Clean ==========
rm(list = ls(all = TRUE))   ## Fjern alt i miljøet – gjør scriptet reproduserbart
...
## Sørger for at cache-mappen finnes selv om prosjektet er nylig klonet
dir.create(dirname(path_cache_rds), recursive = TRUE, showWarnings = FALSE)

## Hard-stopp tidlig hvis input eller konfig mangler – bedre enn å feile halvveis
...
## Normaliser til UTF-8 og gjør NA til tom streng
...
## Slå opp kortnavn fra YAML-aliasser; fallback med ASCII-lower substring
alias_institution_short <- function(inst_name_utf8, cfg_aliases) {
  ...
  ## fallback-regler (substring på ASCII-lower)
  fallback_map <- c(
  ...
  map_one <- function(x) {
    ...
    nx <- norm_utf8(x)
    ## eksakt alias-treff på UTF-8-navn
...
message(sprintf("Lest input: %s | rader: %d, kolonner: %d",
                ...
                nrow(raw), ncol(raw)))
## YAML brukes her kun til alias-oppslag; URL-mønstre håndteres i senere steg
...
## Tolerér variasjoner for "Årstall"
...
## Tolerér varianter for undervisningsspråk/nivå
...
# ===== 6) Filtrer til 2024 =====
## Modul 01 skal kun produsere basis for ett år (2024 nå).
TARGET_YEAR <- 2024L
...
## UTF-8 + trim på sentrale tekstfelter
...
## “Streng” nøkkel: inst + kode (upper) + år + H/V – fanger reelle kollisjoner innen samme semester
...
message("Duplikater (på nøkkel inst_short|code_upper|year|semester_hv): ", n_dup)
if (n_dup > 0) {
  message("Eksempler på duplikatnøkler (inntil 10):")
  print(utils::head(unique(dup_key[dup_flag]), 10))
}

# ===== 10b. Deduplisering (operativ) =====
## For URL-generering ønsker vi kun én rad per institusjon+emnekode – behold første forekomst
...
message(sprintf("Deduplisering: beholdt %d rader (%d fjernet som duplikater på inst+kode).",
                n_after, n_before - n_after))

# ===== 11) Skriv cache (RDS) =====
## Én “sannhet” for resten av pipeline: renset, normalisert og deduplisert per inst+kode
saveRDS(courses_std, path_cache_rds)
message("🟢 Lagret: ", normalizePath(path_cache_rds, winslash = "/"))

## Hurtigsjekk av felter og typer – fjern i batch-kjøring om du vil ha helt stille logging
str(courses_std)
